...
async function loadTranscript(lang) {
    ...
    loadResources(lang, window.RESOURCES_BASE);
}
...
async function loadResources(lang, resourcesBasePath) {
    ...
    var _strings = {
        en: {
            ...
            translationRef: 'Slides & Materials',
            download: 'Download',
            listenOn: 'Listen on',
            openPdf: 'Open PDF'
        },
        es: {
            ...
            translationRef: 'Diapositivas y Materiales',
            download: 'Descargar',
        ...
        ne: {
            ...
            translationRef: 'स्लाइड र सामग्री',
            download: 'डाउनलोड',
            listenOn: 'सुन्नुहोस्',
            openPdf: 'PDF खोल्नुहोस्'
        }
    };
    ...
    try {
        ...
        if (fullPdfs.length > 0 || compactPdfs.length > 0) {
            ...
            if (pdfsEl) {
                ...
                fullPdfs.forEach(function(r) {
                    ...
                    // Koshas extension: a PDF with a `cover` (first-page thumbnail) renders as a
                    // cover card (same style as audio) instead of the plain icon+link row.
                    if (r.cover) {
                        var coverSrc = _basePath + encodeURIComponent(r.cover) + '?v=1';
                        var descP = (_lang === 'ne' && r.description_ne) ? r.description_ne : (_lang === 'es' && r.description_es) ? r.description_es : (r.description || '');
                        pdfsHtml += `
                        <div class="resource-audio-card resource-pdf-card">
                            <img class="resource-cover" src="${coverSrc}" alt="${r.title}" data-modal="true">
                            <div class="resource-body">
                                <div class="resource-title">${r.title} ${badge}</div>
                                <div class="resource-meta">${descP ? descP + ' &middot; ' : ''}<a class="resource-download" href="${fileSrc}" target="_blank" rel="noopener">${s.openPdf} &rarr;</a></div>
                            </div>
                        </div>`;
                    } else {
                        pdfsHtml += `
                ...
                // Koshas extension: PDF cover thumbnails open in the same image modal as audio covers.
                pdfsEl.querySelectorAll('img[data-modal]').forEach(function(img) {
                    img.addEventListener('click', function() {
                        var modal = document.getElementById('img-modal');
                        var modalImg = document.getElementById('img-modal-img');
        ...
        // Koshas extension: "book"/"reference" further-reading cards (cover + author + external links).
        var bookItems = data.resources.filter(function(r) { return r.type === 'book' || r.type === 'reference'; });
        if (bookItems.length > 0) {
            var pdfsEl2 = document.getElementById('resources-pdfs');
            if (pdfsEl2) {
                var frLabel = { en: 'Further Reading', es: 'Lecturas recomendadas', ne: 'थप पढाइ' }[_lang] || 'Further Reading';
                var bhtml = '<div class="resources-heading">' + frLabel + '</div>';
                bookItems.forEach(function(r) {
                    var cover = r.cover ? _basePath + encodeURIComponent(r.cover) + '?v=1' : '';
                    var bdesc = (_lang === 'ne' && r.description_ne) ? r.description_ne : (_lang === 'es' && r.description_es) ? r.description_es : (r.description || '');
                    var meta1 = [r.author, bdesc].filter(Boolean).join(' &middot; ');
                    var links = (r.links || []).map(function(l) { return '<a class="resource-download" href="' + l.url + '" target="_blank" rel="noopener">' + l.label + ' &rarr;</a>'; }).join(' &middot; ');
                    bhtml += '<div class="resource-audio-card resource-book-card">' +
                        (cover ? '<img class="resource-cover" src="' + cover + '" alt="' + r.title + '" data-modal="true">' : '') +
                        '<div class="resource-body"><div class="resource-title">' + r.title + '</div>' +
                        (meta1 ? '<div class="resource-meta">' + meta1 + '</div>' : '') +
                        (links ? '<div class="resource-meta">' + links + '</div>' : '') +
                        '</div></div>';
                });
                pdfsEl2.innerHTML += bhtml;
                pdfsEl2.querySelectorAll('.resource-book-card img[data-modal]').forEach(function(img) {
                    img.addEventListener('click', function() {
                        var modal = document.getElementById('img-modal');
                        var modalImg = document.getElementById('img-modal-img');
                        if (modal && modalImg) { modalImg.src = img.src; modalImg.alt = img.alt; modal.classList.add('open'); }
